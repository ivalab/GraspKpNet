...
class HoleFilling_Filter(object):
	...
	"""
	def __init__(self, flag='min',radius=5,min_valid_depth=100, max_valid_depth=4000,
	             min_valid_neighbors=1,max_radius=20):
		"""
...
class Denoising_Filter(object):
	...
	def _axial_noise_pf(self, z, theta):
		...
		z = z / 1000
		theta = math.pi * theta / 180
		sigma = 1.5 - 0.5 * z + 0.3 * np.power(z, 2) + 0.1 * np.power(z, 3 / 2) * np.power(theta, 2) / np.power(
			math.pi / 2 - theta, 2)
		return sigma
