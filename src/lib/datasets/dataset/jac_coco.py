...
from datasets.dataset.utils import _bbox_overlaps, rotate_bbox
...
class JAC_COCO_36(data.Dataset):
    ...
    mean = np.array([0.711156, 0.709059, 0.197465],
                    dtype=np.float32).reshape(1, 1, 3)
    std = np.array([0.060364, 0.067899, 0.087001],
                   dtype=np.float32).reshape(1, 1, 3)
    ...
    def __init__(self, opt, split):
        ...
        if split:
            self.img_dir = os.path.join(self.data_dir, 'coco/512_cnt_angle', split,
        ...
        self.avg_h = 20.
        self.class_name = [
            "__background__", "orient01", "orient02", "orient03", "orient04", "orient05", "orient06",
            "orient07", "orient08", "orient09", "orient10", "orient11", "orient12", "orient13", "orient14",
            "orient15", "orient16", "orient17", "orient18", "orient19", "orient20", "orient21", "orient22",
            "orient23", "orient24", "orient25", "orient26", "orient27", "orient28", "orient29", "orient30",
            "orient31", "orient32", "orient33", "orient34", "orient35", "orient36"]
        self._valid_ids = [
    ...
    def run_eval_db_middle(self, results):
        ...
        for image_id, result in results.items():
            ...
            for anno in annotations:
                ...
                bbox_gt = anno['bbox']
                x_min, y_min, w, h = bbox_gt[0], bbox_gt[1], \
                                     bbox_gt[2], bbox_gt[3]
                ...
                boxes_gt.append([c_x - w / 2, c_y - h / 2, \
                                       c_x + w / 2, c_y + h / 2, angle])
            ...
            for i in range(len(boxes_s_pr)):
                x_c, y_c = (boxes_pr[i][0] + boxes_pr[i][2]) / 2, \
                           (boxes_pr[i][1] + boxes_pr[i][3]) / 2

                w = np.sqrt(np.power(boxes_pr[i][0]-boxes_pr[i][2], 2) +
                            np.power(boxes_pr[i][1]-boxes_pr[i][3], 2))

                if boxes_pr[i][0] == boxes_pr[i][2] and boxes_pr[i][1] < boxes_pr[i][3]:
                    angle = np.pi / 2
                elif boxes_pr[i][0] == boxes_pr[i][2] and boxes_pr[i][1] > boxes_pr[i][3]:
                    angle = -np.pi / 2
                elif boxes_pr[i][1] == boxes_pr[i][3]:
                    angle = 0
                else:
                    angle = np.arctan((boxes_pr[i][3]-boxes_pr[i][1])/(boxes_pr[i][2]-boxes_pr[i][0]))
            ...
            overlaps = _bbox_overlaps(np.ascontiguousarray(bbox_pr[:, :4], dtype=np.float32),
