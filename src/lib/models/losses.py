...
def _ori_loss(bl_reg, br_reg, bl_tag, br_tag, ori_reg_gt, mask, width, height):
    ...
    ori_loss = nn.functional.smooth_l1_loss(ori_reg_pr * mask.float(), ori_reg_gt * mask.float(), size_average=False)
...
def _ip_loss(tl_reg, bl_reg, br_reg, tl_tag, bl_tag, br_tag, mask, width, height):
    ...
    inner_product = ((bl_xs - tl_xs) * (bl_xs - br_xs) + (bl_ys - tl_ys) * (bl_ys - br_ys))
...
class OriRegLoss(nn.Module):
  ''' nn.Module warpper for orienatation regression loss'''
  def __init__(self):
  ...
  def forward(self, bl_reg, br_reg, bl_tag, br_tag, ori_reg, mask):
      ...
      return self.ori_loss(bl_reg, br_reg, bl_tag, br_tag, ori_reg, mask, width, height)

class InnerProductLoss(nn.Module):
  ''' nn.Module warpper for inner product loss'''
  def __init__(self):
  ...
  def forward(self, tl_reg, bl_reg, br_reg, tl_tag, bl_tag, br_tag, mask):
    ...
    return self.ip_loss(tl_reg, bl_reg, br_reg, tl_tag, bl_tag, br_tag, mask, width, height)

class TagLoss_2(nn.Module):
  '''nn.Module warpper for pull and push loss'''
  def __init__(self):
