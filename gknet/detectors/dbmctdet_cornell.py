...
from utils.post_process import dbmctdet_cornell_post_process
from utils.debugger import Debugger

from .base_detector import BaseDetector

class DbMCtdetDetector_Cornell(BaseDetector):
  def __init__(self, opt):
    super(DbMCtdetDetector_Cornell, self).__init__(opt)
  ...
  def process(self, images, return_time=False):
    with torch.no_grad():
      ...
      dets = dbmctdet_decode(lm, rm, ct, lm_tag, rm_tag, lm_reg, rm_reg, ct_reg, \
                                kernel=self.opt.nms_kernel, ae_threshold=self.opt.ae_threshold, scores_thresh=0.0,
