...
class HoleFilling_Filter(object):
    ...
    def __init__(
        ...
        if flag == "fmi":
            inpaint_fn = partial(
                cv2.inpaint, inpaintRadius=radius, flags=cv2.INPAINT_TELEA
            )
        elif flag == "ns":
            inpaint_fn = partial(
                cv2.inpaint, inpaintRadius=radius, flags=cv2.INPAINT_NS
            )
        else:
            inpaint_fn = None
    ...
    def _get_neighbors(self, img, x, y, radius, img_h, img_w):
        ...
        :return: (valid_neighbors,valid_num)
                        valid_neighbors: list, valid neighboring value
    ...
    def statistical_smoothing(self, image):
        ...
        invalid_x, invalid_y = np.where(image == 0)

        for x, y in zip(invalid_x, invalid_y):
            _r = self.radius
            ...
            if len(valid_neighbors) > 0:
                smoothed[x, y] = self.cal_fn(valid_neighbors)
...
class Denoising_Filter(object):
    ...
    def _axial_noise(self, z, theta):
        ...
        sigma = (
            0.0012
            + 0.0019 * (z - 0.4) ** 2
            + (0.0001 / np.sqrt(z)) * (theta**2 / (math.pi / 2 - theta) ** 2)
        )
        sigma = sigma * 1000
        return sigma
    ...
    def anisotropic_filter(
        self, img, niter=1, kappa=50, gamma=0.1, step=(1.0, 1.0), sigma=0, option=1
    ):
        """
    ...
    def modeling_filter(
        ...
        h, w = img.shape
        lateral_noise = (
            self._lateral_noise_pf if noise_type == "pf" else self._lateral_noise
        )
            ...
            for y in range(w):
                ...
                if D_u >= depth_min and D_u <= depth_max:
                    ...
                    if len(D_k_list) > 0:
                        ...
                        denoised_img[x, y] = (
                            np.sum(D_k_list * w_k_list) / w_k_list.sum()
                        )
        return denoised_img
